...
def evaluate_triple(config,
    ...
    print("Compute Scores:")
    r1 =  calculate_scores_triple_dirichlet_entropy(query_features1, reference_features1, reference_features2, query_features2, reference_features3, query_features3, query_labels1, reference_labels1, step_size=step_size, ranks=ranks) 
...
def calculate_scores_triple_dirichlet_entropy(query_features1, reference_features1, 
                                              reference_features2, query_features2,
                                              reference_features3, query_features3, 
                                              query_labels, reference_labels,
                                              step_size=1000, ranks=[1, 5, 10],
                                              T=1):
    """
    Dirichlet α + 熵驱动的三分支融合
    """
    device = query_features1.device
    topk = copy.deepcopy(ranks)
    Q = query_features1.size(0)
    R = reference_features1.size(0)
    batch_size = min(step_size, Q)
    query_labels_np = query_labels.cpu().numpy()
    reference_labels_np = reference_labels.cpu().numpy()
    ref2index = {idx: i for i, idx in enumerate(reference_labels_np)}
    topk.append(R // 100 if R > 0 else 0)
    results = torch.zeros(len(topk), device=device)

    all_weights = torch.zeros((Q, 3), device=device)

    bar = tqdm(range(0, Q, batch_size), desc="Calculating Dirichlet + entropy weights")
    for start in bar:
        ...
        for i in range(batch_size_actual):
            global_idx = start + i  # 全局索引
            query_label = query_labels_np[global_idx]

            # 获取当前查询的真实相似值，如果标签不存在则跳过
            if query_label not in ref2index:
                continue
            gt_idx = ref2index[query_label]
